#include<reg51.h>
sbit read = P3^0; //read pin of ADC connected to pin 3.0
sbit write = P3^1; //write pin of ADC connected to pin 3.1
sbit intr = P3^2; //interpt pin of ADC connected to pin 3.2
sbit rs = P3^6; //RS pin of LCD
sbit en = P3^7; //enable pin of LC
sbit speaker = P3^4; //speaker
sfr l_data=0xA0;
...
void lcdinit(void); //function to give commands to lcd
 ...
 { if(speaker ) //if the Speaker is ON
 { speaker = 0; //Turn it OFF
...
void Init()
 ...
 SetTimer(0xF1,0x359); //Set the values of TH0 and TL0 for a delay of 3.75ms
 }
...
void Ext0()
 {
 read = 0; //Set the RD pin of ADC from HIGH to LOW //The ADC sends the converted value to P1
 ...
 speaker = 0; //Turn the Speaker OFF
 }
 }
